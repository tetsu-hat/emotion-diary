...
<div>
  ...
  <form enctype="multipart/form-data" action="<?php echo $base_url ?>/diary/register/<?php echo $this->escape($date); ?>" method="post">
    <article>
      <!-- (年月日曜日を表示する箇所) -->
      <h2><?php if(isset($date)){echo $this->escape(date('Y年 m/d (D)',strtotime($date)));}else{echo '取得失敗';} ?></h2>
      ...
      <?php for($i=0;$i<5;$i++):?>
        <article>
          <p>
            ...
            <label>感情</label>
            <select name="emotion_<?php echo $i ?>">
              <?php if(isset($kinds['emotions'])): ?>
                <?php for($k=0;$k<count($kinds['emotions']);$k++):?>
                  <option value="<?php echo $k;?>" <?php if(isset($date_contents[$i]['emotion_id'])&&($kinds['emotions'][$k]['id']==$date_contents[$i]['emotion_id'])){echo 'selected';}else{echo '取得できない';}?>>
